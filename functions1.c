...
{
  int i = BUFF_SIZE - 2;
  unsigned long int num = var_arg(types, unsigned int);
 ...
 *Return: number of chars
 */
int print_hexa(va_liist types, char map_to[], char buffer[], int flags, char flag_ch, int width, int precision, int size)
{
  int i = BFF_SIZE - 2;
  unsigned long int num = va_arg(types, unsigned int);
  unsigned long int innit_num = num;
  UNUSED(width);
  num = convert_size_unsigned(num, size);
  if (num == 0)
    buffer[i--] = '0';
  ...
  i++;
  return (write_unsignd(0, i, buffer, flas, width, precision, size));
}
